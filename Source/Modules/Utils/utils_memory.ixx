...
namespace ge
{
	...
	public:
		...
		constexpr ref_base( const ref_base< other_ptr_t >& other ) requires( std::is_constructible_v<
			ptr_t, const other_ptr_t& > );

		template<typename other_ptr_t>
		constexpr ref_base( ref_base< other_ptr_t >&& other ) noexcept requires( std::is_constructible_v<
			ptr_t, other_ptr_t&& > );

		template<typename other_ptr_t>
		constexpr ref_base& operator=( const ref_base< other_ptr_t >& other ) requires( std::is_assignable_v<
			ptr_t, const other_ptr_t& > );

		template<typename other_ptr_t>
		constexpr ref_base& operator=( ref_base< other_ptr_t >&& other ) noexcept requires( std::is_assignable_v<
			ptr_t, other_ptr_t&& > );

		template<typename other_t>
		constexpr operator shared_ref< other_t >() const requires ( std::is_convertible_v<
			const ptr_t&, typename shared_ref< other_t >::ptr_type > );
