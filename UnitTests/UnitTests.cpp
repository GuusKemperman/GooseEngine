...
namespace smart_ref_tests
{
	...
	TEST_CLASS(smart_refs)
	{
	public:
		TEST_METHOD(ref_base_constructor_nullptr_throwsInvalidArgument)
		{
			try
			{
			...
			{
				Assert::AreEqual(std::string{ "Null pointer passed to reference constructor" }, std::string{ e.what() });
			}
			catch (...)
			{
		...
		TEST_METHOD(shared_ref_partialDeduction_assignmentFromTypedInstance_deducesCorrectly)
		{
			ge::shared_ref ref = ge::make_shared_ref<test_struct>(99); // Deduction here
			Assert::AreEqual(ref->get_value(), 99);
		}

		TEST_METHOD(shared_ref_conversionFromUniqueRef_thenConstAssigns_maintainsCorrectValue)
		{
			...
			Assert::AreEqual(*const1, 123);
			Assert::AreEqual(*const2, 123);
			Assert::AreEqual(*const3, 123);
		}

		TEST_METHOD(shared_ptr_conversionFromUniquePtr_thenConstAssigns_maintainsCorrectValue)
		{
			...
			Assert::AreEqual(*const1, 456);
			Assert::AreEqual(*const2, 456);
			Assert::AreEqual(*const3, 456);
		}

		TEST_METHOD(make_shared_ref_validArgs_constructsExpectedSharedRef)
		{
			auto ref = ge::make_shared_ref<test_struct>();
			Assert::AreEqual(ref->get_value(), 42);
			Assert::AreEqual((*ref).get_value(), 42);
			Assert::AreEqual(ref.get().get_value(), 42);
			Assert::IsTrue(ref.use_count() == 1);
		}

		TEST_METHOD(make_unique_ref_validArgs_constructsExpectedUniqueRef)
		{
		...
		TEST_METHOD(shared_ref_copyConstruct_copiesAndSharesOwnership)
		{
			auto ref1 = ge::make_shared_ref<test_struct>();
			auto ref2 = ref1;
			Assert::AreEqual(ref1.get().get_value(), ref2.get().get_value());
			Assert::AreEqual(ref1.use_count(), 2L);
			Assert::AreEqual(ref2.use_count(), 2L);
		}

		TEST_METHOD(shared_ref_moveConstruct_movesOwnership)
		{
			auto ref1 = ge::make_shared_ref<test_struct>();
			long count = ref1.use_count();
			auto ref2 = std::move(ref1);
			Assert::AreEqual(ref2.use_count(), count);
		}

		TEST_METHOD(shared_ref_copyAssign_copiesAndSharesOwnership)
		{
			auto ref1 = ge::make_shared_ref<test_struct>();
		...
		TEST_METHOD(shared_ref_moveAssign_movesOwnership)
		{
			auto ref1 = ge::make_shared_ref<test_struct>();
			ge::shared_ref<test_struct> ref2 = ge::make_shared_ref<test_struct>();
			ref2 = std::move(ref1);
			Assert::AreEqual(ref2->get_value(), 42);
		}

		TEST_METHOD(unique_ref_moveConstruct_transfersOwnership)
		{
			auto ref1 = ge::make_unique_ref<test_struct>();
			auto val = ref1->get_value();
			auto ref2 = std::move(ref1);
			Assert::AreEqual(ref2->get_value(), val);
		}

		TEST_METHOD(unique_ref_moveAssign_transfersOwnership)
		{
			auto ref1 = ge::make_unique_ref<test_struct>();
		...
		TEST_METHOD(ref_base_getPtr_rvalueAndLvalue_yieldsCorrectPointer)
		{
			auto ref = ge::make_shared_ref<test_struct>();
			...
			Assert::IsNotNull(moved_ptr.get());
		}

		TEST_METHOD(ref_base_operatorConversionToElementRef_returnsReference)
		{
			auto ref = ge::make_shared_ref<test_struct>();
			test_struct& s = ref;
			Assert::AreEqual(s.get_value(), 42);
		}

		TEST_METHOD(ref_base_operatorConversionToSharedRef_compatibleTypes_conversionSucceeds)
		{
			auto base = ge::make_shared_ref<test_struct>();
			ge::shared_ref<const test_struct> converted = base;
			Assert::AreEqual(converted->get_value(), 42);
		}
	};
