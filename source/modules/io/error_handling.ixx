...
namespace ge
{
	...
	struct basic_format_with_trace : format_with_location< types_t... >
	{
		template< typename arg_t >
			requires( std::is_constructible_v< format_with_location< types_t... >, const arg_t, const std::source_location& > )
		consteval basic_format_with_trace(
			const arg_t& a_str,
	...
	class logged_unexpected : public std::unexpected< std::string >
	{
	public:
		API logged_unexpected(
			logger& a_logger,
			std::string_view a_msg,
			const std::source_location& a_src = std::source_location::current() );
...
ge::exception::exception( logger* a_logger, format_with_trace< args_t... > a_format, args_t&&... a_args )
	: exception( a_logger, std::format( a_format, std::forward< args_t >( a_args )... ), a_format.m_src, a_format.m_stacktrace )
{
}
...
ge::logged_unexpected::logged_unexpected( logger& a_logger, std::string_view a_msg, const std::source_location& a_src )
