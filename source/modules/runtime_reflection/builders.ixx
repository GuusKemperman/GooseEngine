...
namespace ge::refl::builders
{
	...
	protected:
		...
		API registry_builder& get_registry_builder() const
		{
			return m_registry_builder;
		}
	...
	public:
		...
		data_builder( const type_builder< outer_t >& prev, std::string_view name )
			: builder_base( prev ),
			  m_target(
				  get_registry().m_datas.push_back(
					  data_data{
	...
	public:
		static_assert(
			std::is_base_of_v< type_builder< typename data_builder< DataPtr >::outer_t >, Prev >,
	...
	export API endable_registry_builder begin_registry()
	{
		return {};
	}
}
